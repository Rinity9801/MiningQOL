...
public class PickaxeCooldownCategoryScreen extends Screen {
    ...
    private static class ToggleControl implements Control {
        ...
        public void render(DrawContext context, int x, int y, boolean hovered, int mouseX) {
            ...
            context.drawBorder(x, y, CONTROL_WIDTH, CONTROL_HEIGHT, hovered ? 0xFF4A90E2 : 0xFF404040);

            int indicatorColor = enabled ? 0xFF44AAFF : 0xFF404040;
            ...
            context.drawTextWithShadow(textRenderer, statusText, statusX, labelY, enabled ? 0xFF44AAFF : 0xFF808080);
        }
    ...
    private static class SliderControl implements Control {
        ...
        public void render(DrawContext context, int x, int y, boolean hovered, int mouseX) {
            ...
            context.fill(sliderX + 1, sliderY + 1, sliderX + fillWidth - 1, sliderY + 7, 0xFF4A90E2);
    ...
    public PickaxeCooldownCategoryScreen(Screen parent) {
        super(Text.literal("Pickaxe Cooldown Settings"));
        this.parent = parent;
    }
    ...
    private void setupControls() {
        ...
        controls.add(new ToggleControl("Enable Pickaxe Cooldown Display",
            PickaxeCooldownHUD.isEnabled(), () -> {
                PickaxeCooldownHUD.setEnabled(!PickaxeCooldownHUD.isEnabled());
                ((ToggleControl) controls.get(0)).enabled = PickaxeCooldownHUD.isEnabled();
            }));

        controls.add(new ButtonControl("Set HUD Position", () -> {
            if (this.client != null) {
                this.client.setScreen(new PickaxeCooldownPositionScreen(this));
            }
        }));

        controls.add(new ToggleControl("Enable \"Almost Ready\" Title",
            PickaxeCooldownHUD.isTitleEnabled(), () -> {
                PickaxeCooldownHUD.setTitleEnabled(!PickaxeCooldownHUD.isTitleEnabled());
                ((ToggleControl) controls.get(2)).enabled = PickaxeCooldownHUD.isTitleEnabled();
            }));

        controls.add(new SliderControl("Title Threshold", 1, 10,
            PickaxeCooldownHUD.getTitleThreshold(), "s", () -> {
                SliderControl slider = (SliderControl) controls.get(3);
                PickaxeCooldownHUD.setTitleThreshold(slider.value);
            }));
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawBorder(windowX, windowY, WINDOW_WIDTH, WINDOW_HEIGHT, 0xFF4A90E2);
        context.drawBorder(windowX + 1, windowY + 1, WINDOW_WIDTH - 2, WINDOW_HEIGHT - 2, 0x884A90E2);
        ...
        String desc = "Displays pickaxe ability cooldown from tab list";
    ...
    private void drawBackButton(DrawContext context, int mouseX, int mouseY) {
        ...
        context.drawBorder(buttonX, buttonY, BACK_BUTTON_WIDTH, BACK_BUTTON_HEIGHT,
                backButtonHovered ? 0xFF4A90E2 : 0xFF404040);
