...
public class AutoClickerCategoryScreen extends Screen {
    ...
    public AutoClickerCategoryScreen(Screen parent) {
        super(Text.literal("CoalClick Settings"));
        this.parent = parent;
    }
    ...
    private void setupControls() {
        ...
        controls.add(new ToggleControl("Show HUD",
            AutoClickerHUD.isEnabled(), () -> {
                AutoClickerHUD.setEnabled(!AutoClickerHUD.isEnabled());
                ((ToggleControl) controls.get(0)).enabled = AutoClickerHUD.isEnabled();
            }));

        controls.add(new ToggleControl("Use Tab Cooldown",
            AutoClickerManager.isUsingTabCooldown(), () -> {
                AutoClickerManager.setUseTabCooldown(!AutoClickerManager.isUsingTabCooldown());
                ((ToggleControl) controls.get(1)).enabled = AutoClickerManager.isUsingTabCooldown();
            }));

        controls.add(new SliderControl("Mining Slot", 0, 8,
            AutoClickerManager.getMiningSlot(), "", () -> {
                SliderControl slider = (SliderControl) controls.get(2);
                AutoClickerManager.setMiningSlot(slider.value);
            }));

        controls.add(new SliderControl("Maniac Miner Cooldown", 50, 150,
            AutoClickerManager.getManiacMinerCooldown(), "s", () -> {
                SliderControl slider = (SliderControl) controls.get(3);
                AutoClickerManager.setManiacMinerCooldown(slider.value);
            }));

        controls.add(new ToggleControl("Enable Rod Swap",
            AutoClickerManager.isRodSwapEnabled(), () -> {
                AutoClickerManager.setEnableRodSwap(!AutoClickerManager.isRodSwapEnabled());
                ((ToggleControl) controls.get(4)).enabled = AutoClickerManager.isRodSwapEnabled();
            }));

        controls.add(new ToggleControl("Enable Second Drill",
            AutoClickerManager.isSecondDrillEnabled(), () -> {
                AutoClickerManager.setEnableSecondDrill(!AutoClickerManager.isSecondDrillEnabled());
                ((ToggleControl) controls.get(5)).enabled = AutoClickerManager.isSecondDrillEnabled();
            }));

        controls.add(new SliderControl("Second Drill Slot", 0, 8,
            AutoClickerManager.getSecondDrillSlot(), "", () -> {
                SliderControl slider = (SliderControl) controls.get(6);
                AutoClickerManager.setSecondDrillSlot(slider.value);
            }));
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawBorder(windowX, windowY, WINDOW_WIDTH, WINDOW_HEIGHT, 0xFFFF4444);
        context.drawBorder(windowX + 1, windowY + 1, WINDOW_WIDTH - 2, WINDOW_HEIGHT - 2, 0x88FF4444);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) {
            ...
            int startX = windowX + (CONTROL_WIDTH - CONTROL_WIDTH) / 2;
            int startY = windowY + 50;
