...
class MinerOverlayCategoryScreen(private val parentScreen: Screen) : VexelScreen("Efficient Miner Overlay Settings") {
    ...
    override fun afterInitialization() {
        ...
        mainPanel = Rectangle(
            ...
            .setSizing(550f, Size.Pixels, 400f, Size.Pixels)
            .childOf(window)
            .apply {
                dropShadow = true
                shadowBlur = 40f
        ...
        mainPanel.yConstraint = (mainPanel.screenHeight - 400f) / 2f
        ...
        // Title
        Text("Efficient Miner Overlay", 0xFFFFFFFF.toInt(), 28f, true)
            .setPositioning(0f, Pos.ParentCenter, 18f, Pos.ParentPixels)
            .childOf(mainPanel)
            .fadeIn(700, EasingType.EASE_OUT)

        // Subtitle
        Text("Highlight players with Efficient Miner perk", 0xFF888888.toInt(), 13f, false)
            .setPositioning(0f, Pos.ParentCenter, 50f, Pos.ParentPixels)
            .childOf(mainPanel)
            .fadeIn(800, EasingType.EASE_OUT)

        // Miner overlay toggles
        val toggles = listOf(
            Triple("Enable Efficient Miner Overlay", "Shows players with Efficient Miner perk", 0xFFFF9944.toInt()) to
                Triple({ EfficientMinerOverlay.isEnabled() },
                       { EfficientMinerOverlay.setEnabled(!EfficientMinerOverlay.isEnabled()) },
                       0xFFFF9944.toInt()),
            Triple("Use Old Heatmap Style", "Classic heatmap rendering instead of beacon", 0xFFFF9944.toInt()) to
                Triple({ EfficientMinerOverlay.isUsingOldHeatmap() },
        ...
        toggles.forEachIndexed { index, (info, actions) ->
            val (label, desc, accentColor) = info
            val (getEnabled, toggleAction, color) = actions
            val delay = 200L + (index * 100L)
            createToggleCardWithDesc(
                label,
                desc,
                color,
                getEnabled,
    ...
    private fun createToggleCardWithDesc(
        label: String,
        description: String,
        accentColor: Int,
        getEnabled: () -> Boolean,
        ...
        Text(description, 0xFF888888.toInt(), 12f, false)
            .setPositioning(20f, Pos.ParentPixels, 38f, Pos.ParentPixels)
            .childOf(card)
