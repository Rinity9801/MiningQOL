...
class MiscCategoryScreen(private val parentScreen: Screen) : VexelScreen("Misc Settings") {
    ...
    override fun afterInitialization() {
        ...
        // Title
        Text("Misc Settings", 0xFFFFFFFF.toInt(), 28f, true)
            .setPositioning(0f, Pos.ParentCenter, 18f, Pos.ParentPixels)
            .childOf(mainPanel)
            .fadeIn(700, EasingType.EASE_OUT)

        // Subtitle
        Text("Miscellaneous features and utilities", 0xFF888888.toInt(), 13f, false)
        ...
        // Toggles
        val toggles = listOf(
            Triple("Auto-skip /sho load", "Automatically runs /sho skipto 1 after /sho load", 0xFF88AAFF.toInt()) to
                Triple({ MiningqolClient.getConfig().autoSkipShoLoad },
                       { MiningqolClient.getConfig().autoSkipShoLoad = !MiningqolClient.getConfig().autoSkipShoLoad },
                       0xFF88AAFF.toInt()),
            Triple("Glass Pane Sync", "Fixes gemstone pane connections when mining (like 1.8.9)", 0xFFAAFF88.toInt()) to
                Triple({ MiningqolClient.getConfig().glassSyncEnabled },
                       {
